...
Change history for **BEA - Content Sync Fusion**, a WordPress multisite plugin for content synchronization.

Entries are preserved from the project’s historical changelog.
...
* Feature: Added synchronization of Gutenberg ACF blocks
...
* Feature: New `exclude_default` mode for attachments. Excludes attachments from the default synchronization and adds a meta box to include them.
...
* Bugfix: Make the synchronization of terms with parents using the parent receiver id
...
## [3.2] — November–December 2017 → April 2018
...
* Add filter `bea_csf.client.post_type.allow_bidirectional_sync` to allow bidirectional synchronization
...
* Remove media synchronization using symlink. Use shared uploads folder.

...
class BEA_CSF_Admin_Synchronizations_Network {
	...
	public static function admin_enqueue_scripts( $hook_suffix = '' ) {
		if ( isset( $hook_suffix ) && 'content-sync_page_bea-csf-add' === $hook_suffix ) {
	...
	public static function render_page_add() {
		// Edition or add ?
		$edit = ( isset( $_GET['action'] ) && 'edit' === $_GET['action'] && isset( $_GET['sync_id'] ) ) ? true : false;

		// if edit, merge array
		if ( true === $edit ) {

			$current_sync = BEA_CSF_Synchronizations::get( [ 'id' => $_GET['sync_id'] ] );
			if ( false === $current_sync ) {
				wp_die( esc_html__( 'This synchronization ID not exists. Tcheater ?', 'bea-content-sync-fusion' ) );
			}
			...
			if ( isset( $_POST['sync'] ) ) {
				...
				$sync_post = wp_unslash( $_POST['sync'] );
			}

			$current_sync_fields = wp_parse_args( $sync_post, self::$_default_fields );
	...
	public static function admin_init() {
		...
		if ( isset( $_GET['page'], $_GET['action'], $_GET['sync_id'] ) && 'bea-csf-edit' === $_GET['page'] && 'delete' === $_GET['action'] ) { // Delete

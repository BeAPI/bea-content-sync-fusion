...
class BEA_CSF_Admin_Blog {
	...
	public function __construct() {
		...
		add_action( 'network_sites_updated_message_resync_bea_csf_content', array( __CLASS__, 'network_sites_updated_message' ) );
		add_action( 'network_sites_updated_message_sync_bea_csf_content', array( __CLASS__, 'network_sites_updated_message' ) );
	}
	 ...
	 * @return array
	 */
	public static function manage_sites_action_links( array $actions, int $blog_id, string $blogname ) {
	...
	public static function wpmuadminedit() {
		...
		if ( isset( $_GET['action'] ) && 'resync_bea_csf_content' === $_GET['action'] && $id > 0 ) {
			...
			exit();
		} elseif ( isset( $_GET['action'] ) && 'sync_bea_csf_content' === $_GET['action'] && $id > 0 ) {
	 ...
	 * @return string
	 */
	public static function network_sites_updated_message( string $msg = '' ): string {
		if ( isset( $_GET['updated'] ) && 'resync_bea_csf_content' === $_GET['updated'] ) {
			return __( 'Resync website OK.', 'bea-content-sync-fusion' );
		} elseif ( isset( $_GET['updated'] ) && 'sync_bea_csf_content' === $_GET['updated'] ) {

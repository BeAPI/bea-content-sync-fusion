...
class BEA_CSF_Client_Taxonomy {
	...
	public static function merge( array $term, array $sync_fields ) {
		...
		// Term exists ?
		$local_term_id = BEA_CSF_Relations::get_post_for_any( $term['blogid'], $sync_fields['_current_receiver_blog_id'], $term['term_id'], $term['term_id'] );
		if ( ! empty( $local_term_id ) && (int) $local_term_id > 0 ) {
			...
			if ( is_wp_error( $new_term_id ) && $new_term_id->get_error_code() == 'term_exists' ) {
				...
				if ( false != $term_exists_result ) {
					$local_term_id = BEA_CSF_Relations::get_post_id_for_receiver( $sync_fields['_current_receiver_blog_id'], $term['blogid'], (int) $term_exists_result['term_id'] );

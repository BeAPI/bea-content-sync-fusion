...
class BEA_CSF_Client_Taxonomy {
	...
	public static function merge( array $term, array $sync_fields ) {
		...
		BEA_CSF_Relations::merge( 'taxonomy', $term['blogid'], $term['term_taxonomy_id'], $GLOBALS['wpdb']->blogid, $new_term_obj->term_taxonomy_id );
	...
	public static function delete( array $term, array $sync_fields ) {
		...
		if ( $local_tt_id > 0 ) {
			...
			if ( $local_term_id > 0 ) {
				...
				BEA_CSF_Relations::delete_by_receiver( 'taxonomy', $GLOBALS['wpdb']->blogid, $local_tt_id );
			}
		}

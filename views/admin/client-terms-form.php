		...
		<?php if ( self::is_valid_blog_id($_origin_blog_id) ) : ?>
			...
			<select id="emitter-blog" name="term_emitter[blog_id]">
				<?php foreach( BEA_CSF_Admin_Synchronizations_Network::get_sites_from_network() as $site ) : ?>
					<option value="<?php echo esc_attr($site['blog_id']); ?>" <?php selected($site['blog_id'], $_origin_blog_id, true); ?>><?php echo esc_html($site['blogname'] . ' ('.$site['domain'].$site['path'].')'); ?></option>
				<?php endforeach; ?>
			</select>
			...
			<select name="term_emitter[term_taxonomy_id]">
				...
				<?php foreach( get_terms( $_origin_taxonomy, array('hide_empty' => false) ) as $term ) : ?>
					<option value="<?php echo esc_attr($term->term_taxonomy_id); ?>" <?php selected($term->term_taxonomy_id, $_origin_term_taxonomy_id, true); ?>><?php echo esc_html($term->name); ?></option>
				<?php endforeach; ?>
			</select>

			...
			<?php foreach( $syncs as $sync_obj ) : 
				...
				<select class="widefat multiple-helper" name="sync_notifications[<?php echo $sync_obj->get_field('id'); ?>][]" multiple="true">
					<?php foreach( $users as $user ) : ?>
						<option value="<?php echo esc_attr($user->ID); ?>" <?php selected(true, in_array($user->ID, $current_values[$sync_obj->get_field('id')])); ?>><?php echo esc_html($user->user_login); ?></option>
					<?php endforeach; ?>
				</select>
